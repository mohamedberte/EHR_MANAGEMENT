...
router.post('/issuer_server', async function(req, res, next) {
    ...
    switch(req.body.type_request){
        case "ATTRS_REQUEST" :
            ...
            newRequest.request(req.body.param2.did, req.body.param2.verkey, nonce, req.body.type_request,'PENDING', JSON.stringify(req.body.param2.data), req.body.endpoint_to, req.body.endpoint_from);
        ...
        case "CHALLENGE_REPLY" :
            ...
            Request.findOne({did: did}, async function(err, request){
                if (request === null) {
                    ...
                    var hash = crypto.createHash('sha256', request.nonce).digest('base64');
                    console.log("hash = " +  hash);

                    var result =  await indy.cryptoVerify(request.verkey, Buffer.from(hash, 'utf8'), Buffer.from(data_signed.data))
                    if(result){
            ...
            case "CREDENTIAL_REQUEST":
                ...
                var newRequest = new Request();
                newRequest.request(req.body.data.credRequest.prover_did, "Already now", req.body.req_id, req.body.type_request,'PENDING', JSON.stringify(req.body.data), req.body.endpoint_to, req.body.endpoint_from);
                console.log(newRequest);
                newRequest.save((err, request) => {
                    if (err){                
                        data = "Request rejected";
